...
Namespace Controllers
    Public Class ManagerController
        ...
        Function PurchaseOrderPage() As ActionResult
            ...
            purchaseOrders = (From p In db.purchased_order
                              Where p.status = True
                              Select p).ToList()

            Return View("~/Views/managers/PurchaseOrder.vbhtml", purchaseOrders)
        End Function
        ...
        <HttpPost>
        <ValidateAntiForgeryToken()>
        Function UpdatePrice(detailId As Integer, orderId As Integer, price As Integer) As ActionResult
            ...
            Dim purchaseOrderDetail As purchased_order_detail = Nothing
            purchaseOrderDetail = (From p In db.purchased_order_detail
                                   Where p.id = detailId And p.purchased_order_id = orderId
                                   Select p).FirstOrDefault()
            ...
            Dim purchaseOrder As purchased_order = Nothing
            purchaseOrder = db.purchased_order.Find(purchaseOrderDetail.purchased_order_id)

            purchaseOrder.total_price = purchaseOrder.total_price + price
        ...
        Function DeleteDetail(detailId As Integer, orderId As Integer) As ActionResult
            ...
            db.purchased_order_detail.Remove(purchaseOrderDetail)
            db.SaveChanges()

            Dim purchaseOrder As purchased_order = Nothing
            purchaseOrder = db.purchased_order.Find(purchaseOrderDetail.purchased_order_id)

            purchaseOrder.total_price = purchaseOrder.total_price - purchaseOrderDetail.price
            db.Entry(purchaseOrder).State = EntityState.Modified
            db.SaveChanges()
        ...
        <HttpPost>
        <ValidateAntiForgeryToken()>
        Function CheckoutPurchaseOrder(orderId As Integer) As ActionResult
        ...
        Function PurchaseOrderDetail(id As Integer) As ActionResult
            ...
            Dim purchaseOrderDetails As List(Of purchased_order_detail) = Nothing
            purchaseOrderDetails = (From p In db.purchased_order_detail
                                    Where p.purchased_order_id = id
                                    Select p).ToList()

            Dim purchaseOrder As purchased_order = Nothing
            purchaseOrder = db.purchased_order.Find(id)

            Dim tupleData As New Tuple(Of List(Of purchased_order_detail), purchased_order)(purchaseOrderDetails, purchaseOrder)


            Return View("~/Views/managers/PurchaseOrderDetail.vbhtml", tupleData)
        End Function

        Function CartReminderPage() As ActionResult
            ...
            If carts IsNot Nothing Then
                For Each item In carts
                    Dim user As user = db.user.Find(item.user_id)
                    users.Add(user)
                Next
            End If

            Return View("~/Views/managers/CartReminder.vbhtml", users)
        End Function

        <HttpPost>
        <ValidateAntiForgeryToken()>
        Function CartReminder(mailTo As String, content As String) As ActionResult
            Dim senderEmail As String = "truongvanphon020297@gmail.com"
            Dim senderPassword As String = "0961654996a"
            Dim receiverEmail As String = "phon-tv@fanatic.vn"
            Dim subject As String = "Hello from VB.NET"
        ...
        Public Function ExportToExcel() As ActionResult

            Dim data As List(Of order) = db.order.ToList()
            ...
            excelWorkSheet.Cells(1, 1) = "ORDER ID"
            excelWorkSheet.Cells(1, 2) = "REGISTER TIME"
            excelWorkSheet.Cells(1, 3) = "TOTAL PRICE"

            For i As Integer = 0 To data.Count - 1
                excelWorkSheet.Cells(i + 2, 1) = data(i).id

                excelWorkSheet.Cells(i + 2, 2) = data(i).register_time
                excelWorkSheet.Cells(i + 2, 3) = data(i).total_price
            Next


            Dim fileName As String = "YourFileName.xlsx"
