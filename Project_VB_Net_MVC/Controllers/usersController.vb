...
Namespace Controllers
    Public Class usersController
        ...
        Function LoginOTP() As ActionResult
        ...
        <HttpPost()>
        <ValidateAntiForgeryToken()>
        Function SendOTP(phone As String) As ActionResult
            Dim accountSid As String = ""
            Dim authToken As String = ""
            Dim twilioPhoneNumber As String = ""

            TwilioClient.Init(accountSid, authToken)


            Dim otpCode As String = "123456"

            Dim messageOptions = New CreateMessageOptions(New PhoneNumber(twilioPhoneNumber)) With {
        ...
        Public Function FindNearest() As ActionResult
        ...
        Public Function FindNearest(latitude As Double, longitude As Double) As JsonResult
            ...
            Dim nearestStore As storage = FindNearestStore(currentLocation)

            If nearestStore IsNot Nothing Then
                Return Json(New With {.message = $"Nearest store is: {nearestStore.Name}, Address: {nearestStore.Address}"})
            Else
                Return Json(New With {.message = "No stores found."})
            End If
        End Function

        Private Function FindNearestStore(currentLocation As GeoCoordinate) As storage
            Dim nearestStore As storage = Nothing
            Dim shortestDistance As Double = Double.MaxValue
            Dim stores As List(Of storage) = db.storage.ToList()

            For Each store As storage In stores
                Dim storeLocation As New GeoCoordinate(store.latitude, store.longitude)
                Dim distance As Double = currentLocation.GetDistanceTo(storeLocation)
